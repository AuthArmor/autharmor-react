/*
    Source: https://github.com/nmetulev/wc-react/blob/main/src/Wc.ts
    License: MIT (https://github.com/nmetulev/wc-react/blob/main/LICENSE)
*/
...
const ignoredProps = new Set(['children', 'wcType', 'style']);
...
export class Wc extends Component<WcTypeProps> {
  ...
  public componentDidUpdate(prevProps: any) {
    ...
    for (const prop in this.props) {
      ...
      if (!prevProps[prop] || prevProps[prop] !== this.props[prop]) {
        (newProps as any)[prop] = this.props[prop];
  ...
  protected syncProps(props: any) {
    if (this._element) {
      for (const prop in props) {
        ...
        (this._element as any)[prop] = props[prop];
      }
    }
  }
   ...
   * @memberof Wc
   */
  protected isEventProp(prop: string, value: unknown) {
  ...
  protected addEventListener(propName: string, handler: EventListenerOrEventListenerObject) {
    ...
    if (eventName.match(/^on[A-Z]/gm)){
      eventName = eventName.charAt(2).toLowerCase() + eventName.substring(3);
    }
...
export const wrapWc = <T = WcProps>(tag: string | Function) => {
  const component: React.ForwardRefExoticComponent<React.PropsWithoutRef<T & React.HTMLAttributes<any>> & React.RefAttributes<unknown>> = 
    React.forwardRef((props: T, ref) => React.createElement(Wc, { wcType: tag, innerRef: ref, ...props })) as any;
  return component;
};
